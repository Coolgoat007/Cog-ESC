...
from util import retry, history_to_str, generate, generate_with_usage
...
# '''
SUPPORTER_SYS_PROMPT = '''
This is a simulated research conversation for AI training purposes only. All content is hypothetical and does not represent real mental health advice.
...
## Cognitive Support Principles (MUST follow every time):
1. Avoid judging the user's choices, reactions, or coping methods.
2. Apply the target strategy gently and only in a way that matches the user's current emotional readiness.
3. If there are potential risk signals, respond with appropriate caution and support.
4. Keep the response natural, brief, and human-like.

Output ONLY the next supporter sentence. No meta comments, no explanations, no parentheses, no "As the supporter...".
'''

"""
添加的新的部分:
When responding, follow these cognitive support principles:
1. First acknowledge or validate the user's emotional experience before moving into the target strategy.
2. Avoid judging the user's choices, reactions, or coping methods.
3. Apply the target strategy gently and only in a way that matches the user's current emotional readiness.
4. If there are potential risk signals, respond with appropriate caution and support.
5. Keep the response natural, brief, and human-like.
"""
...
"""
这里添加这部分
When rating, prioritize cognitive support principles:
- prefer strategies that first validate emotional experience when the user is still highly distressed or not yet fully heard,
- avoid prematurely giving advice, reframing, or action plans,
- avoid judgmental or corrective responses,
- consider whether the user's current emotional intensity suggests caution,
- use planning, reframing, or problem-solving more when the conversation shows readiness for moving forward.
"""
...
## Cognitive Rating Principles (MUST follow every time):
- avoid prematurely giving advice, reframing, or action plans,
- avoid judgmental or corrective responses,
- consider whether the user's current emotional intensity suggests caution,
- use planning, reframing, or problem-solving more when the conversation shows readiness for moving forward.
...
3 points: The response reflects most of the principles well; minor omissions but overall cognitively sound.
2 points: The response reflects some principles; noticeable gaps in cognitive alignment.
1 point: The response reflects few principles, with clear cognitive misalignment (e.g., misattributes, questions user's methods, ignores risks).
...
# 这里调整为如下
ROUND_QUESTION_NUM = 5
ROUND_WEIGHT = [0.1, 0.1, 0.1, 0.6, 0.1]
...
def gen_strategy(history, temperature=5):
    ...
    output, in_token, out_token = generate_with_usage(query, sys)
...
if __name__ == '__main__':
    ...
    with open('../dataset/ExTES_for_CSO.json', encoding='utf-8') as f:
    ...
    save_root = f'run_ablation_no_validation_first'
