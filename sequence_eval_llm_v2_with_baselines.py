...
OUT_DIR = "sequence_eval_with_baselines_results"
...
def evaluate(name, path, max_workers=20):
    ...
    summary_file = os.path.join(OUT_DIR, f"{name}_summary.txt")
...
def main():
    print("\n" + "="*60)
    print("Sequence Evaluation with Strong Baselines")
    print("="*60)

    summaries = []
    for name, path in FILES.items():
        result = evaluate(name, path, max_workers=30)
        if result:
    ...
    print(f"\n{'='*60}")
    print(f"All summaries saved to: {out_csv}")
    print(f"{'='*60}\n")
    ...
    print("SUMMARY TABLE")
    print("="*60)
    print(f"{'Model':<20} {'Avg Length':<12} {'Val-First %':<15} {'Premature %':<15}")
    print("-"*60)
    for name, n, avg_len, vf, pa in summaries:
        print(f"{name:<20} {avg_len:<12.2f} {vf*100:<15.2f} {pa*100:<15.2f}")
    print("="*60)
