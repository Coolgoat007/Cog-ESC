...
def generate_with_usage(query, system=None):
    global openai
    if openai.is_closed():
        openai = OpenAI(
            api_key=os.getenv('openai_key'),
    ...
    if hasattr(chat_completion, "usage") and chat_completion.usage is not None:
        in_token = getattr(chat_completion.usage, "prompt_tokens", 0) or 0
        out_token = getattr(chat_completion.usage, "completion_tokens", 0) or 0

    return output, in_token, out_token
